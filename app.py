...
@app.route('/')
def serve_index():
    # Serve index.html from the parent folder (CropDiseaseDetector)
    return send_from_directory('..', 'index.html')

@app.route('/<path:filename>')
def serve_static(filename):
    # Serve css and js files from the parent folder
    return send_from_directory('..', filename)

@app.route('/api/analyze', methods=['POST'])
def analyze():
    # 1. Stop if the API key hasn't been added
    if not API_KEY:
        return jsonify({"status": "error", "message": "API Key missing. Please configure GEMINI_API_KEY in Render Environment Variables!"}), 500
